...
class promotionService {
    ...
    function getInfo($seq) {
        $db = new ConnectDB();
        $strSql = " select * from tb_promotion where i_promotion =" . $seq;
        $_data = $db->Search_Data_FormatJson($strSql);
        $db->close_conn();
        return $_data;
    }
    ...
    function getInfoFile($db) {
        $strSql = "select s_img_p1 img from tb_promotion ";
        $strSql .= " union ";
        $strSql .= " select s_img_p2 img from tb_promotion ";
        $_data = $db->Search_Data_FormatJson($strSql);
        return $_data;
    }

    function delete($db, $seq) {
        $strSQL = "DELETE FROM tb_promotion WHERE i_promotion = '" . $seq . "' ";
        $arr = array(
            array("query" => "$strSQL")
        );
    ...
    function deleteAll($db, $query) {
        ...
        $strSQL = "DELETE FROM tb_promotion WHERE i_promotion in ($query) ";
        $arr = array(
            array("query" => "$strSQL")
        );
        $reslut = $db->insert_for_upadte($arr);
    ...
    function SelectByArray($db, $query) {
        $strSql = "SELECT * FROM tb_promotion WHERE i_promotion in ($query) ";
        $_data = $db->Search_Data_FormatJson($strSql);
        return $_data;
    }
