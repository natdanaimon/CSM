...
class processService {
    ...
    function dataTable() {
        ...
        $strSql .= " and s.s_status not in ('A','C','R1','R2','R3')";
        $strSql .= " ) tb_cust ,";
        $strSql .= " (";
    ...
    function addDismantling($db, $info, $filename) {
        $strSql = "";
        $strSql .= "INSERT ";
        $strSql .= "INTO ";
        $strSql .= "  tb_img_dismantling ( ";
        $strSql .= "    ref_no, ";
        $strSql .= "    s_image, ";
        $strSql .= "    d_create, ";
        $strSql .= "    s_create_by ";
    ...
    function delDismantling($db, $info) {
        $strSQL = "DELETE FROM tb_img_dismantling WHERE ref_no = '" . $info[ref_no] . "' and s_image = '$info[filename]' ";
        $arr = array(
            array("query" => "$strSQL")
        );
        $reslut = $db->insert_for_upadte($arr);
