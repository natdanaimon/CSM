...
function getTurnOver($u, $d) {
    $db2 = new ConnectDB();
    $strSql2 = "select sum(f_turnover) f_turnover from tb_cs_excel where s_user = '$u' and s_date_range = '$d'";
...
function sumLv1_Percen($u, $d) {
    return (getSumLv1_Sports($u, $d) * 0.002) + ((getSumLv1_Casino($u, $d) / 7) * 2);
}

function sumLv2_Percen($u, $d) {
    return (getSumLv2_Sports($u, $d) * 0.001) + (getSumLv2_Casino($u, $d) / 7);
}

function sumTotal() {
    return sumLv1_Percen($u, $d) + sumLv2_Percen($u, $d);
}
