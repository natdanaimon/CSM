...
//            $util->CopyTemplatedMail("../email/Email_Register.html", "../email/Email_Register_Temp.html", $info[detail]);
    ...
    public function dataTable() {
        ...
        if ($_dataTable != NULL) {
            foreach ($_dataTable as $key => $value) {
                $_dataTable[$key]['s_phone'] = $this->setPhoneOnDisplay($_dataTable[$key]['s_phone']);
            }
    ...
    public function setPhoneOnDisplay($phone) {
        $pFirst = substr($phone, 1, 3);
        $pCenter = substr($phone, 6, 5);
        $pLast = substr($phone, 11);
        if ($_SESSION["perm"] == "U") {
            return "XXX" . "-" . $pCenter . "XXX";
        } else {
            return $pFirst . "-" . $pCenter . $pLast;
        }
    }
...
//            echo $return2099;
        } else if (!$util->isEmpty($info[s_email]) && !filter_var($info[s_email], FILTER_VALIDATE_EMAIL)) {
...
//            echo $return2099;
        } else if ($util->isEmpty($info[f_special_bonus])) {
            $return2099 = eregi_replace("field", $_SESSION['lb_cs_special_bonus'], $return2099);
            echo $return2099;
            ...
            echo $return2097;
        } else if ($util->isEmpty($info[s_username]) && $info[status] == "APPR") {
