...
function initialDataTable(first) {
    $.ajax({
        type: 'GET',
        url: 'controller/employee/employeeController.php?func=dataTable',
        beforeSend: function ()
        {
...
//                    col_checkbox += ' </label>';
                ...
                col_checkbox += '  value="' + item.i_emp + '" onclick=remove_select_all("checkbox_' + i + '")>';
                ...
                if (item.s_image != "") {
                    col_picture = '<a title="' + item.s_image + '" class="example-image-link" href="upload/employee/' + item.s_image + '" data-lightbox="example-' + item.i_emp
                            + '">';
                    col_picture += '<img class="example-image" src="upload/employee/' + item.s_image + '" width="32px" height="32px"  />';
                    col_picture += '</a>';
                }
                ...
                col_edit += '<a href="emp_employeeManage.php?func=edit&id=' + item.i_emp + '" class="btn btn-circle btn-icon-only blue" style="width:32px;height:32px">';
                col_edit += ' <i class="fa fa-edit"></i>';
                col_edit += '</a>';

                col_delete += '<a href="' + (disable != "" ? '#' : 'javascript:Confirm(\'' + item.i_emp + '\',\'delete\');') + '" style="width:33px;height:33px" class="btn btn-circle btn-icon-only red" ' + disable + '>';
...
$(document).on('click', '.notifyjs-foo-base .notify-all-yes', function () {
    ...
    $.ajax({
        type: 'GET',
        url: 'controller/employee/employeeController.php',
...
$(document).on('click', '.notifyjs-foo-base .notify-yes', function () {
    ...
    $.ajax({
        type: 'GET',
        url: 'controller/employee/employeeController.php?func=' + func + '&id=' + id,
        beforeSend: function ()
        {
            $('#se-pre-con').fadeIn(100);
        },
