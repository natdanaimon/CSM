...
function initialDataTable(first) {
    $.ajax({
        type: 'GET',
        url: 'controller/setting/yearController.php?func=dataTable',
        beforeSend: function ()
        {
...
//                var col_code = item.i_year;
                var col_year = (language == "th" ? item.i_year : item.i_year);
                ...
                col_checkbox += '  value="' + item.i_year + '" onclick=remove_select_all("checkbox_' + i + '")>';
                ...
                col_delete += '<a href="' + (disable != "" ? '#' : 'javascript:Confirm(\'' + item.i_year + '\',\'delete\');') + '" class="btn btn-circle btn-icon-only red" ' + disable + '>';
...
$(document).on('click', '.notifyjs-foo-base .notify-all-yes', function () {
    ...
    $.ajax({
        type: 'GET',
        url: 'controller/setting/yearController.php',
...
$(document).on('click', '.notifyjs-foo-base .notify-yes', function () {
    ...
    $.ajax({
        type: 'GET',
        url: 'controller/setting/yearController.php?func=' + func + '&id=' + id,
        beforeSend: function ()
        {
            $('#se-pre-con').fadeIn(100);
        },
