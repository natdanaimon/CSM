...
function initialDataTable(first) {
    $.ajax({
        type: 'GET',
        url: 'controller/setting/vatController.php?func=dataTable',
        beforeSend: function ()
        {
        ...
        success: function (data) {
            ...
            $.each(res, function (i, item) {
                ...
                var col_checkbox = "";
                var col_vat = item.f_vat + "%";
                var col_update = item.d_update;
                var col_status = "";
                var col_edit = "";
                var col_delete = "";
...
//                col_checkbox += '  value="' + item.i_vat + '" onclick=remove_select_all("checkbox_' + i + '")>';
...
//                col_checkbox += '</span>';
                ...
                col_edit += '<a href="set_vatManage.php?func=edit&id=' + item.i_vat + '" class="btn btn-circle btn-icon-only blue">';
...
$(document).on('click', '.notifyjs-foo-base .notify-all-yes', function () {
    ...
    $.ajax({
        type: 'GET',
        url: 'controller/setting/vatController.php',
...
$(document).on('click', '.notifyjs-foo-base .notify-yes', function () {
    ...
    $.ajax({
        type: 'GET',
        url: 'controller/setting/vatController.php?func=' + func + '&id=' + id,
        beforeSend: function ()
        {
            $('#se-pre-con').fadeIn(100);
        },
