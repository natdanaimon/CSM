...
function initialDataTable(first) {
    $.ajax({
        type: 'GET',
        url: 'controller/setting/autoController.php?func=dataTable',
        beforeSend: function ()
        {
        ...
        success: function (data) {
            ...
            $.each(res, function (i, item) {
                ...
                var col_checkbox = "";
                var col_dept = sortIndex(item.i_index) + item.s_dept_th;
                var col_day = item.i_day;
                var col_status = "";
                var col_edit = "";
                var col_delete = "";
...
//                col_checkbox += '  value="' + item.i_dept + '" onclick=remove_select_all("checkbox_' + i + '")>';
...
//                col_checkbox += '</span>';
                ...
                col_edit += '<a href="set_autoManage.php?func=edit&id=' + item.i_dept + '" class="btn btn-circle btn-icon-only blue">';
                ...
                col_delete += '<a href="' + (disable != "" ? '#' : '#') + '" class="btn btn-circle btn-icon-only red" disabled="disabled">';
...
function sortIndex(index) {
    return "<span style='display:none;'>" + index + "</span>";
...
$(document).on('click', '.notifyjs-foo-base .notify-all-yes', function () {
    ...
    $.ajax({
        type: 'GET',
        url: 'controller/setting/autoController.php',
...
$(document).on('click', '.notifyjs-foo-base .notify-yes', function () {
    ...
    $.ajax({
        type: 'GET',
        url: 'controller/setting/autoController.php?func=' + func + '&id=' + id,
        beforeSend: function ()
        {
            $('#se-pre-con').fadeIn(100);
        },
