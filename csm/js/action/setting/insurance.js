...
function initialDataTable(first) {
    $.ajax({
        type: 'GET',
        url: 'controller/setting/insuranceController.php?func=dataTable',
        beforeSend: function ()
        {
        ...
        success: function (data) {
            ...
            $.each(res, function (i, item) {
                ...
                col_checkbox += '  value="' + item.i_ins_comp + '" onclick=remove_select_all("checkbox_' + i + '")>';
                ...
                if (item.s_image != "") {
                    col_picture = '<a title="' + item.s_image + '" class="example-image-link" href="upload/compInsurance/' + item.s_image + '" data-lightbox="example-' + item.i_ins_comp + '">';
                    col_picture += '<img class="example-image" src="upload/compInsurance/' + item.s_image + '" width="50px" height="50px"  />';
                    col_picture += '</a>';
                }else{
                ...
                col_edit += '<a href="set_compInsuranceManage.php?func=edit&id=' + item.i_ins_comp + '" class="btn btn-circle btn-icon-only blue">';
                col_edit += ' <i class="fa fa-edit"></i>';
                col_edit += '</a>';


                col_delete += '<a href="' + (disable != "" ? '#' : 'javascript:Confirm(\'' + item.i_ins_comp + '\',\'delete\');') + '" class="btn btn-circle btn-icon-only red" ' + disable + '>';
...
$(document).on('click', '.notifyjs-foo-base .notify-yes', function () {
    ...
    $.ajax({
        type: 'GET',
        url: 'controller/setting/insuranceController.php?func=' + func + '&id=' + id,
        beforeSend: function ()
        {
            $('#se-pre-con').fadeIn(100);
        },
