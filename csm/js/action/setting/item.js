...
function initialDataTable(first) {
    $.ajax({
        type: 'GET',
        url: 'controller/setting/itemController.php?func=dataTable',
        beforeSend: function ()
        {
        ...
        success: function (data) {
            ...
            $.each(res, function (i, item) {
                ...
                var col_checkbox = "";
                var col_picture = "";
                var col_code = item.s_code;
                var col_item = (language == "th" ? item.s_item_th : item.s_item_th);
                ...
                col_checkbox += '  value="' + item.i_item + '" onclick=remove_select_all("checkbox_' + i + '")>';
                ...
                if (item.s_image != "") {
                    col_picture = '<a title="' + item.s_image + '" class="example-image-link" href="upload/item/' + item.s_image + '" data-lightbox="example-' + item.i_item + '">';
                    col_picture += '<img class="example-image" src="upload/item/' + item.s_image + '" width="50px" height="50px"  />';
                    col_picture += '</a>';
                }else{
                ...
                col_edit += '<a href="set_itemManage.php?func=edit&id=' + item.i_item + '" class="btn btn-circle btn-icon-only blue">';
                col_edit += ' <i class="fa fa-edit"></i>';
                col_edit += '</a>';


                col_delete += '<a href="' + (disable != "" ? '#' : 'javascript:Confirm(\'' + item.i_item + '\',\'delete\');') + '" class="btn btn-circle btn-icon-only red" ' + disable + '>';
...
$(document).on('click', '.notifyjs-foo-base .notify-yes', function () {
    ...
    $.ajax({
        type: 'GET',
        url: 'controller/setting/itemController.php?func=' + func + '&id=' + id,
        beforeSend: function ()
        {
            $('#se-pre-con').fadeIn(100);
        },
