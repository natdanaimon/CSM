...
function initialDataTable(first) {
    $.ajax({
        type: 'GET',
        url: 'controller/setting/brandController.php?func=dataTable',
        beforeSend: function() {
        ...
        success: function(data) {
            ...
            $.each(res, function(i, item) {
                ...
                col_checkbox += '  value="' + item.i_brand + '" onclick=remove_select_all("checkbox_' + i + '")>';
                ...
                if (item.s_image != "") {
                    ...
                    col_picture += '<img class="example-image" src="upload/brand/' + item.s_image + '" width="30px" height="30px"  />';
                    col_picture += '</a>';
                } else {
                    col_picture += '<img class="example-image" src="images/noImage.jpeg" width="30px" height="30px"  />';
                ...
                col_edit += '<a href="set_carbrandManage.php?func=edit&id=' + item.i_brand + '" class="btn btn-circle btn-icon-only blue">';
                col_edit += ' <i class="fa fa-edit"></i>';
                col_edit += '</a>';


                col_delete += '<a href="' + (disable != "" ? '#' : 'javascript:Confirm(\'' + item.i_brand + '\',\'delete\');') + '" class="btn btn-circle btn-icon-only red" ' + disable + '>';
...
$(document).on('click', '.notifyjs-foo-base .notify-all-yes', function() {
    ...
    $.ajax({
        type: 'GET',
        url: 'controller/setting/brandController.php',
...
$(document).on('click', '.notifyjs-foo-base .notify-yes', function() {
    ...
    $.ajax({
        type: 'GET',
        url: 'controller/setting/brandController.php?func=' + func + '&id=' + id,
        beforeSend: function() {
            $('#se-pre-con').fadeIn(100);
        },
        success: function(data) {
...
function openLogs() {
    var myWindow = window.open(pathOpenLogs+"logs/logImportBrand.txt", "", "width=400,height=400");
}
