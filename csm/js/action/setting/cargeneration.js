...
function initialDataTable(first) {
    $.ajax({
        type: 'GET',
        url: 'controller/setting/generationController.php?func=dataTable',
        beforeSend: function ()
        {
        ...
        success: function (data) {
            ...
            $.each(res, function (i, item) {
                ...
                col_checkbox += '  value="' + item.i_gen + '" onclick=remove_select_all("checkbox_' + i + '")>';
                ...
                col_edit += '<a href="set_cargenerationManage.php?func=edit&id=' + item.i_gen + '" class="btn btn-circle btn-icon-only blue">';
                col_edit += ' <i class="fa fa-edit"></i>';
                col_edit += '</a>';


                col_delete += '<a href="' + (disable != "" ? '#' : 'javascript:Confirm(\'' + item.i_gen + '\',\'delete\');') + '" class="btn btn-circle btn-icon-only red" ' + disable + '>';
...
$(document).on('click', '.notifyjs-foo-base .notify-yes', function () {
    ...
    $.ajax({
        type: 'GET',
        url: 'controller/setting/generationController.php?func=' + func + '&id=' + id,
        beforeSend: function ()
        {
            $('#se-pre-con').fadeIn(100);
        },
...
//    console.log($(this).serialize());
    ...
    $.ajax({
        type: 'POST',
        url: 'controller/setting/generationController.php?func=import',
        data: formData,
        cache: false,
        contentType: false,
        processData: false,
