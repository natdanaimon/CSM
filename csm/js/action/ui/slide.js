...
function initialDataTable(first) {
    $.ajax({
        type: 'GET',
        url: 'controller/ui/slideController.php?func=dataTable',
        beforeSend: function ()
        {
        ...
        success: function (data) {
            ...
            $.each(res, function (i, item) {
                ...
                if (item.s_image != "") {
                    col_picture = '<a title="' + item.s_image + '" class="example-image-link" href="upload/slide/' + item.s_image + '" data-lightbox="example-' + item.i_slide + '">';
                    col_picture += '<img class="example-image" src="upload/slide/' + item.s_image + '" width="35px" height="35px"  />';
                    col_picture += '</a>';
                }
                ...
                col_edit += '<a href="ui_slideManage.php?func=edit&id=' + item.i_slide + '" class="btn btn-circle btn-icon-only blue">';
...
$(document).on('click', '.notifyjs-foo-base .notify-yes', function () {
    ...
    $.ajax({
        type: 'GET',
        url: 'controller/ui/slideController.php?func=' + func + '&id=' + id,
        beforeSend: function ()
        {
            $('#se-pre-con').fadeIn(100);
        },
