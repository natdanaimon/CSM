...
function initialDataTable(first) {
    $.ajax({
        type: 'GET',
        url: 'controller/po/createControllerColor.php?func=dataTable',
        beforeSend: function() {
        ...
        success: function(data) {
            ...
            $.each(res, function(i, item) {
                ...
                var col_checkbox = "";
                var col_refno = item.s_po_color_ref;
                var col_license = item.s_firstname+" "+item.s_lastname;
                //var col_name = item.s_firstname+" "+item.s_lastname;
                var col_name = item.s_comp_th;


                var col_caryear = item.s_po_color_ref;
                var col_carbrand = item.s_po_color_ref;
                var col_cargen = item.s_po_color_ref;
                var col_carsub = item.s_po_color_ref;
                var col_insurance = item.s_po_color_ref;
//                var col_dmg = item.i_dmg;
                var col_inout = item.s_po_color_ref;
                ...
                col_checkbox += '  value="' + item.i_po_color + '" onclick=remove_select_all("checkbox_' + i + '")>';
                ...
                col_view += '<a href="po_color_views.php?func=edit&id=' + item.i_po_color + '" class="btn btn-circle btn-icon-only blue" style="width:32px;height:32px" target="_blank">';
                col_view += ' <i class="fa fa-eye"></i>';
                col_view += '</a>';
                
                col_edit += '<a href="po_color_createManage.php?func=edit&id=' + item.i_po_color + '" class="btn btn-circle btn-icon-only blue" style="width:32px;height:32px">';
                col_edit += ' <i class="fa fa-edit"></i>';
                col_edit += '</a>';

                col_delete += '<a href="' + (disable != "" ? '#' : 'javascript:Confirm(\'' + item.i_po_color + '\',\'delete\');') + '" style="width:33px;height:33px" class="btn btn-circle btn-icon-only red" ' + disable + '>';
...
$(document).on('click', '.notifyjs-foo-base .notify-all-yes', function() {
    ...
    $.ajax({
        type: 'GET',
        url: 'controller/repair/createControllerColor.php',
...
$(document).on('click', '.notifyjs-foo-base .notify-yes', function() {
    ...
    $.ajax({
        type: 'GET',
        url: 'controller/repair/createControllerColor.php?func=' + func + '&id=' + id,
        beforeSend: function() {
            $('#se-pre-con').fadeIn(100);
        },
        success: function(data) {
