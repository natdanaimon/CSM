function importFile() {
    $('#form-action-file').submit(function (e) {
        ...
        $.ajax({
            type: 'POST',
            url: 'controller/vipPassController.php',
            data: formData,
            cache: false,
            contentType: false,
            processData: false,
...
function initialDataTable(first) {
    $.ajax({
        type: 'GET',
        url: 'controller/vipPassController.php?func=dataTable',
        beforeSend: function ()
        {
            $('#se-pre-con').fadeIn(100);
        },
        success: function (data) {
            ...
            $.each(res, function (i, item) {
                ...
                var col_username_login = (item.s_username_login != "" ? item.s_username_login : "-");
                var col_pass = item.s_password;
                var col_status = "";
                var col_edit = "";
                var col_delete = "";
                ...
                col_checkbox += '  value="' + item.s_username + '" onclick=remove_select_all("checkbox_' + i + '")>';
                ...
                col_edit += '<a href="cs_vip_pass_manage.php?func=edit&id=' + item.s_username + '" class="btn btn-circle btn-icon-only blue">';
                col_edit += ' <i class="fa fa-edit"></i>';
                col_edit += '</a>';


                col_delete += '<a href="' + (disable != "" ? '#' : 'javascript:Confirm(\'' + item.s_username + '\',\'delete\');') + '" class="btn btn-circle btn-icon-only red" ' + disable + '>';
...
$(document).on('click', '.notifyjs-foo-base .notify-yes', function () {
    ...
    $.ajax({
        type: 'GET',
        url: 'controller/vipPassController.php?func=' + func + '&id=' + id,
        beforeSend: function ()
        {
            $('#se-pre-con').fadeIn(100);
        },
