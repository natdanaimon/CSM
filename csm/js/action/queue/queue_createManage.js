function searchRef() {
  var s_queue_ref = $('#s_queue_ref').val();
  //alert(s_queue_ref);
  if (s_queue_ref.length > 6) {

    $.ajax({
      type: 'GET',
      url: 'controller/queue/createController.php?func=getInfoRef&id=' + s_queue_ref,
      beforeSend: function () {
      ...
      success: function (data) {
        ...
        if (data) {
          ...
          $.each(res, function (i, item) {
            ...
            edit(item.i_cust_car);

          });
        } else {

          //edit(0)
          $.notify("Not found Ref.no " + s_queue_ref, "error");
...
function getDateTotal() {
  ...
  $(".i_dept_date").each(function () {
    if (!isNaN(this.value) && this.value.length != 0) {
      total += parseFloat(this.value);
    }
  });

  $('#total_date').val(total);
  var d_inbound = $('#d_inbound').val();
  $.ajax({
    type: 'GET',
    url: 'controller/commonController.php?func=MNGetDate',
    data: {d_inbound: d_inbound, total: total},
    beforeSend: function () {
      //$('#se-pre-con').fadeIn(100);
    },
    success: function (data) {
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      $.each(res, function (i, item) {
        ...
        //alert(item.s_txt_1)
        if (item.s_txt_1 != '') {
          $('.div_listother_1').show();
        }
        if (item.s_txt_2 != '') {
          $('.div_listother_2').show();
        }
        if (item.s_txt_3 != '') {
          $('.div_listother_3').show();
        }
        if (item.s_txt_4 != '') {
          $('.div_listother_4').show();
        }
        if (item.s_txt_5 != '') {
          $('.div_listother_5').show();
        }
        if (item.s_txt_6 != '') {
          $('.div_listother_6').show();
        }
        if (item.s_txt_7 != '') {
          $('.div_listother_7').show();
        }
        if (item.s_txt_8 != '') {
          $('.div_listother_8').show();
        }
        if (item.s_txt_9 != '') {
          $('.div_listother_9').show();
        }
        if (item.s_txt_10 != '') {
          $('.div_listother_10').show();
        }
        if (item.s_txt_11 != '') {
          $('.div_listother_10').show();
        }
        if (item.s_txt_12 != '') {
          $('.div_listother_12').show();
        }
        if (item.s_txt_13 != '') {
          $('.div_listother_13').show();
        }
